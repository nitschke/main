...
\section{Curvature approximations}
  \subsection{Curvature vector} \label{secCurvatureVector}
    ...
    The equation \eqref{eqCurvatureVector} is component-wise to read and can be discretized with 
    discrete Laplace-Beltrami operator in \eqref{eqDisLaplace}, 
    where \( f \) becomes the \mh{i}{th} coordinate functions \( x^{i} \) for
    \( i=0,1,2 \).
    Hence, we can easily build the local coefficients matrices for the elements \( \sigma^{2} \) 
    with \( A^{\sigma^{2}} \) from \eqref{eqLokalMatrixLaplace}:
    \begin{align}
      \begin{bmatrix}
        A^{\sigma^{2}} & 0 & 0 \\
        0 & A^{\sigma^{2}} & 0 \\
      ...
      \end{bmatrix}
    \end{align}
    to determine the left-hand side of the system
    \begin{align} \label{eqDisCurvatureVector}
      \left\langle *\Delta_{B} x^{i}, \star v \right\rangle
        &= \left\langle *H^{i}, \star v \right\rangle \formpunkt
    \end{align}
    for all vertices \( v\in K \) and \( i=0,1,2 \).
    \( H^{i}\in \R \) are the components of the curvature vector.
    The right-hand side of \eqref{eqDisCurvatureVector} can be locally represented by the decomposed Voronoi area 
    \begin{align}
      C^{\sigma^{2}}_{i} = \left|  \right|
    \end{align}
    around \( v \).
