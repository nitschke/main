...
int main(int argc, char* argv[])
{
  ...
  dxyz.set(new DXYZ_d());
  // harmonic
  DofEdgeVector dt(edgeMesh, "dt");
  dt.set(new DT_d());
  dt.writeFile("output/dt.vtu");
  DOFVector< WorldVector<double> > dtSharp = dt.getSharpFaceAverage();
  io::VtkVectorWriter::writeFile(dtSharp, "output/dtSharp.vtu");




  DofEdgeVector initSol = dxyz + rotxyz + 10*dt;
  ...
  CoBeltrami.setUhOld(initSol);
  dectorus.addMatrixOperator(CoBeltrami, 0, 0);
  dectorus.addVectorOperator(CoBeltrami, 0);
  // Beltrami
  EdgeOperator Beltrami;
  Beltrami.addTerm(new LaplaceBeltramiAtEdges());
  Beltrami.setUhOld(initSol);
  dectorus.addMatrixOperator(Beltrami, 1, 1);
  dectorus.addVectorOperator(Beltrami, 1);
  ...
  dectorus.addVectorOperator(Id, 2);


  dectorus.assembleSystem();

  dectorus.solve();

  dectorus.writeSolution();
