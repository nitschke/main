...
\section{Gauß-Bonnet-Operator}
  ...
  \subsection{Implementierung}
    ...
    \begin{figure}
      \begin{minipage}[t]{0.49\textwidth}
        \centering\input{bilder/tikz/GaussBonnet.tex}
      \end{minipage}
      \hfill
      \begin{minipage}[t]{0.49\textwidth}
        \centering\input{bilder/tikz/ElementGaussBonnet.tex}
      \end{minipage}
    ...
    \( \sigma^{2}=\left[ v, v_{i}, v_{i+1} \right] \) eingeschränkten Voronoi-Kanten bestimmen durch
    \begin{align}
...
\label{secNumEx}
  ...
  Die Gaußsche Krümmung kann aus der diskreten Weingartenabbildung berechnet werden mit (SK) und ohne (SKAvN) gegebenen Normalenfeld 
  oder mit Hilfe des Gauß-Bonnet-Operators (GB)
  und die mittlere Krümmung ebenfalls aus der diskreten Weingartenabbildung (SH)/(SHAvN) oder aus dem Krümmungsvektor, der aus dem diskreten Laplace-Beltrami-Operators der Koordinatenabbildung
  folgt (LX).
  ...
  (S*AvN)-Probleme (\eqref{eqWeingartenEq1}+\eqref{eqWeingartenEq2}) nicht entkoppelt, so dass in der
  ...
  BiCGStab2-Algorithmus benutzt mit einer Abbruchstoleranz von \( 1.0E-10 \).
  ...
  Jedoch liegt die Vermutung nahe, dass die Konvergenzordnung mit feiner werdenden Gitter, unter
  ...
  \end{table}
   ...
   \begin{table}[htbp]
      ...
      \end{tabular}
      \caption[Weingarten auf der Sphäre (S*)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung(S*)) auf der Sphäre.}
      ...
      \end{tabular}
      \caption[Weingarten auf der Sphäre (S*AvN)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung mit gemittelten Normalen(S*AvN)) auf der Sphäre.}
   ...
   \begin{table}[htbp]
      ...
      \end{tabular}
      \caption[Weingarten auf einer quartischen Oberfläche (S*)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung(S*)) auf einer quartischen Oberfläche.}
      ...
      \end{tabular}
      \caption[Weingarten auf der Sphäre (S*AvN)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung mit gemittelten
   ...
   \begin{table}[htbp]
      ...
      \end{tabular}
      \caption[Weingarten auf einem Ellipsoid (S*)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung(S*)) auf einem Ellipsoid.}
      ...
      \end{tabular}
      \caption[Weingarten auf einem Ellipsoid (S*AvN)]{Relative Fehler für \( K \) und \( H \) (diskreten Weingartenabbildung mit gemittelten
