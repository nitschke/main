...
\cite{Lee} und \cite{FirstCourse} nachgelesen werden.
...
Genauso wie auch die äußere Ableitung die Menge der Differentialformen zu solch einen Komplex macht, den de-Rham-Komplex.
Hierbei werden wir den Satz von Stokes ausnutzen, welcher bewirkt, dass sich das Integral der äußere Ableitung als Randintegral ausdrücken
lässt. 
Da die genutzten Integrationsgebiete Ketten sind, ist nicht mehr viel zutun, weil der Randoperator für Ketten schon im vorherigen Kapitel
hergeleitet wurde.
...
Ebenfalls werden wir einen diskreten Hodge-Stern-Operator entwickeln und damit auch den Laplace-Beltrami-Operator erhalten.
...
\section{Diskrete Differentialformen}
  ...
  \begin{definition}
    Die de-Rham-Abbildung bildet \( p \)-Differentialformen auf die diskrete \( p \)"~For"-men (Koketten) ab. 
    Genauer
    \begin{align}
      \begin{aligned}
  ...
  \begin{folgerung}
    ...
    Das hat den Vorteil, dass sich jedes \( \alpha_{d}\in\Omega_{d}^{p}(L) \) auch als \( \psi(\alpha) = \alpha_{d} \) schreiben lässt, weil
    ...
    \begin{proof}
      ...
      Nun brauchen wir für \(i=1,\ldots, N_{\sigma^{p}} \) nur noch geeignete \( \alpha_{i}\in\Omega^{p}(M) \) finden, sodass
      \begin{align}
        \label{eq}
  ...
  \begin{bemerkung}
    ...
    Das entspricht unseren Anforderungen jedoch nur für flache Mannigfaltigkeiten.
    ...
    Rein formal ließe sich die Situation entschärfen, indem wir die Simplizes von \( K \) auf die Mannigfaltigkeit projizieren (vgl. \eqref{diagSingulaerAbstrakt}), also wird
    \( \left\langle \psi^{p}(\alpha), \pi(\sigma^{p}) \right\rangle = \left( \psi^{p}(\alpha) \circ \pi \right)(\sigma^{p}) \) gerechnet für ein \( \sigma^{p}\in K \).
    Nur ist in vielen praktischen Aufgaben weder die Projektion noch die Mannigfaltigkeit exakt bekannt. 
    Somit bleibt also nur die approximative Auswertung des Integrals.
    ...
    In dieser Arbeit werden wir uns größtenteils auf die Auswertung von \( 0 \)-Formen beschränken. 
    Das reicht aus um skalarwertige oder vektorisierte skalarwertige Differentialgleichungen höherer
    ...
    Denn mittels \( \flat \), beziehungsweise \( \sharp \), ist ein Isomorphismus zwischen Vektorfeldern und \( 1 \)-Formen gegeben.
    ...
    \begin{figure}
      ...
      \caption[Approximation einer 1-Form]{Das umliegende Koordinatensystem ist so gedreht, dass die Kante \( \sigma_{\eps}^{1} \) und die zugehörige abstrakte Kante \( s_{\eps} \)
                                           ...
                                           \( \pi \) klebt die Kante auf die Mannigfaltigkeit, somit gilt \( \pi(\sigma_{\eps}^{1}) = s_{\eps}. \)}
      \label{figDiskrete1Form}
    \end{figure}
    Dies soll nicht heißen, dass die \( 1 \)-Form \( \alpha\in\Omega^{1}(M) \) mit mindestens Ordnung 2 approximiert wird, denn im hinteren Faktor steckt noch immer die Größe \( \eps \).
    Zudem hängt \( s_{\eps} \) stark von der Geometrie der Mannigfaltigkeit ab.
    ...
    Damit bleiben nur noch die \( 2 \)-Formen übrig und diese unterscheiden sich wegen dem Hodge-Stern-Isomorphismus nur um einen geometrischen Faktor (und Syntax) von den \( 0 \)-Formen.
...
\section{Äußere Ableitung}
  ...
  \begin{definition}
    ...
    Für eine Mannigfaltigkeit \( M \) und ein Primärgitter \( K \) heißt 
    \begin{align}
  ...
  \begin{folgerung}
    Die diskrete äußere Ableitung lässt sich mit Hilfe des Satzes von Stokes und dem Randoperator berechnen.
    ...
    denn wenn wir \mbox{\( \alpha_{1},\alpha_{2} \in \Omega^{p}(M) \)}
    mit \( \psi(\alpha_{1}) = \psi(\alpha_{2}) \) wählen, 
  ...
  \begin{bemerkung}
    Wir betrachten nun im Speziellen wieder ein zweidimensionales Pri"-mär"-gitter \( K \), beziehungsweise dessen Dualgitter \( \csd K \), 
...
\label{secHodgeStar}
  ...
  mit lokalen orthogonalen Koordinaten \( \left( x^{1}, x^{2} \right) \) und metrischem Tensor \( g=\text{diag}(g_{1}, g_{2})\).
  ...
  Hierbei merken wir schon, dass der Hodge-Stern-Operator durchaus auch eine anschauliche Seite besitzt.
  ...
  Wie die diskreten \( 1 \)-Formen auf die Basisvektoren reagieren, wissen wir schon aus \eqref{eqApprox1Form}.
  ...
  \end{align}
  Da in \( \frac{\partial}{\partial x^{2}} \)-Richtung integriert wird, findet auch nur der zweite Summand Beachtung.
  \begin{align}
    \psi\left( *\alpha \right)(\star\sigma^{1}) = \int_{\pi(\star\sigma^{1})} \sqrt{g^{1}g_{2}} v_{1} dx^{2}
  \end{align}
  Des Weiteren sei eine Kurve \( s_{\delta}:\left[ -\delta_{1}, \delta_{2} \right] \rightarrow \pi(\star\sigma^{1}) \subset M\) gegeben, sodass
  ...
  \begin{definition}
    ...
    Wir werden im Folgenden auch hier nur kurz \( \Omega_{d}^{n-p}(\star K)\) für das Bild \( *\Omega_{d}^{p}(K)\le \Omega_{d}^{n-p}(\csd K)\) schreiben.
  \end{definition}
  ...
  \begin{proof}
    ...
    \end{align}
    auf \( |K| \) mit den Koordinaten \(\alpha_{i_{1}\ldots i_{p}} \) gegeben. 
    ...
    \begin{align}
      \left| (*\psi(\alpha))(\star\sigma^{p}) - |\star\sigma^{p}|\alpha_{1 \ldots p}(c) \right|
            ...
            &\le  |\star\sigma^{p}| \left( \|\nabla\alpha_{1 \ldots p}(c)\| \eps_{\sigma^{p}} + \mathcal{O}(\eps_{\sigma^{p}}^{2}) \right) \formpunkt
    \end{align}
    Wenn \( p = 0 \) ist, dann gilt für \( \alpha:= f \in C^{\infty}(M) \) die Gleichheit
    \begin{align}
      (*\psi(f))(\star\sigma^{0}) = |\star\sigma^{0}|f(c) \formpunkt
    \end{align}
    Für die Dualkette \( \star\sigma^{p} \) mit \( 0\le p < n \) lässt sich eine zu \eqref{eq1} ähnlich Abschätzung finden.
    ...
    Mit der Länge der längsten in der Dualkette \( \star\sigma^{p} \) enthaltenen Kante
    \begin{align}
  ...
  \begin{folgerung}
    ...
    \begin{align}
      ...
      \left| (*\psi(\hat\alpha))(\sigma^{p}) - \psi(*\hat\alpha)(\sigma^{p})\right| \le \left| \sigma^{p} \right| \mathcal{O}\left(\eps_{\sigma^{p}} + \hat{l}_{\star\sigma^{p}}\right)
    \end{align}
  ...
  auf der exakten Form \( \exd\alpha\in\Omega^{p+1} \) verursacht, da wir für die äußere Ableitung wegen des Stokes-Theorems keinen Fehler machen.
  Anders sieht die Situation für die Koableitung
  \begin{align}
    \label{eqKoableitungStetig}
    \delta = (-1)^{np+1}*\exd *: \Omega^{p+1}(M) \rightarrow \Omega^{p}(M)
  \end{align}
  aus, bei der wir zweimal den Hodge-Stern-Operators diskretisieren müssen.
  ...
  das heißt es gilt 
  \begin{align}
   \left\langle\left\langle \exd\alpha, \beta \right\rangle\right\rangle = \left\langle\left\langle \alpha, \delta\beta \right\rangle\right\rangle \formpunkt
  \end{align}
  In \cite{Marsden} ist die Koableitung so definiert. 
  In anderen Lektüren, wie zum Beispiel \cite{jaenich}, ist \( -\delta \) der adjungierte Operator.
  Das kann zu einiger Verwirrung führen. 
  Deshalb einigen wir uns, hier den in \eqref{eqKoableitungStetig} angegeben Operator für die Koableitung zu verwenden.
  ...
  \begin{bemerkung}[zur Implementierung]
    Prinzipiell können alle hier vorgestellten Operatoren als Matrizen implementiert werden, da sie linear sind. 
    In \cite{siggraphKap7} und \cite{siggraphKap8} wird dies zum Beispiel vorgeschlagen.
    Das wollen wir hier nicht machen, da in Zukunft auch nichtlineare Terme behandelt und das Interface und die Methodik von AMDiS genutzt werden sollen.
    ...
    Die Problemformulierung, das Aufaddieren der Elementmatrizen, sowie das Lösen des Gleichungssystems bleibt somit im Ursprünglichen Zustand erhalten.
  ...
  \begin{fazit}
    ...
    Für alle \({ f\in C^{\infty}(M) }\) definiert sich die Rotation über
    \begin{align}
    ...
    In \cite{desbrun} werden zwei diskrete Dachprodukte auf \( \Omega_{d}^{p}(K)\times\Omega_{d}^{q}(K) \) bzw. \( \Omega_{d}^{p}(\star K)\times\Omega_{d}^{q}(\star K) \)
    ...
    Die Schwierigkeit an dem resultierenden (Tangential-)Vektorproblem ist, dass wir noch keine diskreten
    Übersetzungsisomorphismen \( \sharp \) und \( \flat \) haben.
    Diese gilt es in zukünftigen Arbeiten sinnvoll zu entwickelt.
    ...
    Wollen wir mit \( 1 \)-Formen oder mit Vektorfeldern speicherintern arbeiten und bei Bedarf die Indizes herrauf bzw. herunter ziehen?
    ...
    Bei Vektorfeldern wird es da schon schwieriger. 
    Am sichersten ist es, diese auf den Dreieckelementen zu speichern (effektiv 2 Freiheitsgrade pro Element), da es dort einen eindeutigen
    ...
    führen. 
    Es müssen, für ein eigentlich zweidimensionales Problem, drei Freiheitsgrade für die Ecken angesetzt werden, was zu einem überbestimmten System führen könnte.
    ...
    Auf dem Primärgitter ließe sich diese Frage noch am einfachsten beantworten, da dort auf einem Simplex, nach dem Mittelwertsatz der Differentialrechnung, der Tangentialraum am
    wenigsten einem Punkt auf dem zugehörigen abstrakten Simplex übereinstimmt, d.h. dort hätten wir zumindest 
    einen gemeinsamen Vektorraum zum Vergleichen.
    ...
    Weitere Möglichkeiten zum Vergleichen für zweidimensionale Probleme sind zum einen den Ambienteraum \( \R^{3} \) zu nutzen, in welchem \( M \) und \( |K| \) eingebettet sind.
    Hier ist die Schwierigkeit, dass zum Beispiel der Hodge-Operator \( 1 \)-Formen auf \( 2 \)-Formen abbildet. 
    Das heißt, es muss im Limes auch sichergestellt werden, dass \( * \) auch
    ...
    Die äußere Ableitung ist aber auch metrikunabgängig. 
    Der Hodge-Stern-Operator ist das im Allgemeinen nicht und damit haben wir solch eine Invarianz gegenüber dem Pullback für eine 
...
\section{Laplace-Operator}
  ...
  \begin{ziel}
    Wenn es um Numerik partieller Differentialgleichungen geht, liegt der Schwerpunkt oftmals auf der Diskretisierung des Laplace-Operators bzw. Gleichungen der Form
    \begin{align}
      \label{eqAllgemeineLaplaceGleichung}
      \Delta u &= F(u)
    \end{align}
    mit zum Beispiel linearem \( F \) und Skalarfeld oder Vektorfeld \( u \).
    Auch zeitabhängige Probleme, die durch eine Methode der Wahl in der Zeit diskretisiert werden 
    und somit in jedem Zeitschritt auch in die Kategorie \eqref{eqAllgemeineLaplaceGleichung} passen, sind denkbar.
    ...
    testen. 
    Des Weiteren kann der Laplace-Operator auch zur Bestimmung der mittleren Krümmung benutzt
  ...
  \subsection{Laplace-Beltrami-Operator}
    ...
    für \( 0 \)-Formen ist Vorsicht geboten, da dies Grund zur Verwirrung sein könnte.
    ...
    \begin{bemerkung}
      ...
      Daraufhin werden die 20 Dreiecke gleichmäßig unterteilt.
      ...
      der aus der Voronoi-Zelle hinausfließt.
      \begin{align}
      \begin{aligned}
  ...
  \subsection{Implementierung}
    ...
    Das elementweise aufsummieren ist korrekt, da für \mbox{\( \sigma^{1}=\left[ v_{i}, v_{j} \right]\)},
    ...
    \( \sigma^{2} \) berechnen, können wir den Anteil der rechten Seite
    ...
    \begin{align}
      ...
      \end{bmatrix}
        &=: A^{\sigma^{2}} f^{\sigma^{2}}
    \end{align}
    mit der symmetrischen Elementmatrix \( A^{\sigma^{2}} \in \R^{3 \times 3} \) 
    und dem Elementvektor \( f^{\sigma^{2}}\in\R^{3} \).
    Da das Primärgitter aus \( N_{\sigma^{0}} \) Ecken besteht,
    lässt sich die Elementmatrix, unter Beachtung der lokalen und globalen Nummerierung der Ecken, 
    auf eine \mbox{\( ( N_{\sigma^{0}} \times N_{\sigma^{0}} ) \)-Matrix}
    \( \bar{A}^{\sigma^{2}} \)erweitern, sodass
    \begin{align}
  ...
  \subsection{Beispiel: Poisson-Gleichung}
    ...
    \begin{beispiel}[Einheitssphäre]
       ...
       \end{align}
       Als Primärgitter nutzen wir einen verfeinerten Ikosaeder.
    ...
    \begin{figure}
      ...
      \caption[Laplace auf Torus]{Links: Gitter mit maximalen Winkeln.
                                         Die dicke schwarze geschlossene Kurve auf dem Torus deutet die die Verdrillung des Torusschlauches um \( 4\pi \) an.
    ...
    \begin{beispiel}[Torus]
      ...
      Wie schon im letzten Beispiel wurde auch hier wieder der BiCGStab2 mit einer Abbruchs"-toleranz von
...
\label{secGradient}
  ...
  1-Form \( \exd f \) die Indizes hinaufzuziehen um ein diskretes Tangentialvektorfeld zu erhalten.
  ...
  \begin{definition}
    ...
    \end{align}
    (mit dem diskreten dualen Tangentialvektorraum \( \mathcal{V}_{d}(\star K) \)) schreiben wir auch kurz \( \nabla^{pd} \).
  ...
  \subsection{Implementierung}
    ...
    \end{align}
    die sich in der gleichen Weise wie im Abschnitt \ref{subsecLaplaceImplementierung} zu einer Systemmatrix \( A_{q} \) aufsummieren lässt, sodass
    \begin{align}
  ...
  \subsection{Beispiele}
    \begin{beispiel}[Sphäre]
      ...
      Der Ikosaeder war das Grundgitter, welches verfeinert wurde um alle anderen Primärgitter zu erzeugen.
    ...
    \begin{figure}
      \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=\textwidth]{bilder/gradSphere/gradAndError1k.png}
      \end{minipage}
      \hfill
      \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=\textwidth]{bilder/gradSphere/error50k.png}
      \end{minipage}
      \caption[Gradient auf Sphäre]
